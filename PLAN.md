...
| P2 | Capture Guard, four calibration tiers, Tier-0 perception on device | Airplane-mode grading of a real photo on a phone | software done and tested in headless Chromium with a real photo as the camera feed; real-phone check is T9 |
| P3 | Receipt, signing, revisions, contest flow, offline verifier, rule-pack time travel | Two-phone verification works | done in headless Chromium (second browser context, offline, empty database); real two-phone check is T9 |
| P4 | Data engine, Tier-1 model, E1-E4 | Holdout report generated; per-class ship decision | Tier-0 and Tier-1 evaluated on public real photos (reports/zenodo_*.json); E1-E4 scripts and templates ready, waiting for field data (T3-T7) |
| P5 | W5 sequential sampling, W6 weight and value, W8 fleet, W9 languages | Each visible in the demo | done |
| P6 | APK, replay demo, docs, pitch | Installable app, demo works in airplane mode | see README |
...
2. T3 photos. Re-fit Tier-0 thresholds on the day-1 set (`tools/fit_tier0.ts`), then run the holdout evaluation on day 2 only.
